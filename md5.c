...
void md5_update(md5_t *m, const char *message, size_t len) {
  WORD32 wbuff[MLEN];
  if (m->bufflen != 0) {
    int numbytes = m->bufflen+len < MD5_BLOCKSIZE ? len :
      MD5_BLOCKSIZE - m->bufflen;
    memcpy(&m->buff[m->bufflen], message, numbytes);
    if ((m->bufflen += numbytes) < MD5_BLOCKSIZE)
      return;
...
void md5_finish(md5_t *m, char output[MD5_HASHSIZE]) {
  ...
  memset(&m->buff[m->bufflen], 0, MD5_BLOCKSIZE-m->bufflen);
  bytestoword32(wbuff, m->buff);
  if (m->bufflen > (MD5_BLOCKSIZE-8)) {
    digest(wbuff, m->d);
    memset(wbuff, 0, MD5_BLOCKSIZE);
  }
  wbuff[MLEN-2] = (m->len<<3) & MASK;
  wbuff[MLEN-1] = (m->len>>(WSIZE-3)) & 0x7;
  digest(wbuff, m->d);
